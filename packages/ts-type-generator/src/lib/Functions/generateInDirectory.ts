 ...
 */
export const generateInDirectory =
  (name: string, generateDocument: OperatorFunction<string, string>) =>
  (directories: string[], discard?: string[]) =>
    filesInDirectory(directories, discard).pipe(
      mergeMap((directory) => {
        return of(directory.content).pipe(
