...
export const generateFunctionByBinary = curryN(
  3,
  (
    length: number,
    template: {
    ...
    if (argsLeft.length === 0) {
      return `${generateTemplate(
        ...
        length
      )}(${args}): ${generateTemplate(template.resultTemplate, length)}\n\n`;
    }
    return `${generateTemplate(
      template.functionTemplate,
      length
    )}(${args}): ${generateFunctionFixed(
      template.curryResultTemplate,
      argsLeft
    )}\n\n`;
  }
);
