...
export const generateFunctionFixed = curry(
  (template: string, numbers: number[]): string => {
    const result = findGeneratorTags(template).reduce(
      (acc, { tag, template: t }) => {
        const generated: string = map(
          populateGeneratorTagTemplate(t),
          numbers
        ).join(', ');
    ...
    return stripEmptyGeneric(
      populateGeneratorTagTemplate(result)(numbers.length)
    );
  }
);
