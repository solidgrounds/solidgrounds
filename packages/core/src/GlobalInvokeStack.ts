...
const addCompilerPass = (compilerPass: CompilerPass) => {
  return getCurrentInvokeWindow().serviceContainer.addCompilerPass(
    compilerPass
  );
};
...
async function asyncInvokeWindow<T>(
  this: unknown,
  window: InvokeWindow<T>,
  callback: (window: InvokeWindow<T>) => Promise<void>
): Promise<void> {
  invokeWindowStack.push(window as InvokeWindow);
  try {
    await callback.call(this, window);
  } finally {
    invokeWindowStack.splice(
      invokeWindowStack.indexOf(window as InvokeWindow),
      1
    );
  }
}

function invokeWindow<T, R = void>(
  this: unknown,
  window: InvokeWindow<T>,
  callback: () => R
): R {
...
const printInvokeStack = () => {
  return invokeWindowStack
    .reverse()
    .map((window, index) => {
      const serviceContainerName = `[${index + 1}]:SC<${
        window.serviceContainer.name
      }>`;
