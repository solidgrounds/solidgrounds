...
export class ServiceContainerFactory<S extends KernelFeatureServices> {
  ...
  ): ServiceContainerFactory<KernelFeatureServices> => {
  ...
  public constructor(
    protected featureGroupFactory: FeatureGroupFactoryInterface,
   ...
   */
  public add<Services, Deps extends Partial<S>>(
    f1: FF<Services, Deps>
  ): ServiceContainerFactory<S & Services> {
    const container = new ServiceContainerFactory(
      new FeatureGroupFactory(),
      this.options
    );
  ...
  public async build(): Promise<S> {
    const featureGroup = this.featureGroupFactory.build<S>(
