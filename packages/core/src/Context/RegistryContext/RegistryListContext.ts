...
export type RegisterListArgument<TDependencies, TType> = KSF<
  TDependencies,
  TType
>;
export type InferListArgumentRegister<T, S> = S extends ListLike<infer TType>
  ? RegisterListArgument<T, TType>
  : never;
export type InferListArgumentsRegister<T, S> = S extends ListLike<infer TType>
  ? RegisterListArgument<T, TType>[]
  : never;

export const isRegisterListArguments = (
  args: unknown[]
): args is RegisterListArgument<unknown, unknown>[] => {
  return typeof args[0] === 'function' || typeof args[0] === 'string';
};

export function isListLike<T>(register: unknown): register is ListLike<T> {
  return (
    isObjectLike(register) &&
    (isSetLike(register as ListLike<T>) ||
      isArrayLike(register as ListLike<T>) ||
      isRegisterLike(register as ListLike<T>))
  );
}
...
export function isArrayLike<T>(
  register: ListLike<T>
): register is ArrayLike<T> {
  return typeof (register as ArrayLike<T>).push === 'function';
}

export function isRegisterLike<T>(
  register: ListLike<T>
): register is RegisterLike<T> {
  return typeof (register as RegisterLike<T>).register === 'function';
}

export function registerToList<T, TService, TType>(
  context: CompileContext<T>,
  parent: SFR<TService>,
  items: RegisterListArgument<T, TType>[]
): TService {
  ...
  if (!isListLike<TType>(arrayLike)) {
    throw new ContainerError(
      `Expected ListLike object, not ${parent.name} (${typeof arrayLike})`
    );
  }
